module Admin
  module STour
    class AddIncludedInPriceOption
      ...
      def valid_params?
        @tour and @tour.options_included_in_price.size < 31 and @tour.options_included_in_price.include?(@option) == false and @option.size > 3 and @option.size < 11
      end

      def add_option_to_list
        @tour.options_included_in_price << @option
        @tour.save
      end
    end
  end
